...
import { RELX_CONFIG, SITE_CONFIG } from '../shared/constant.js'
import { outputFileSyncOnChange } from '../shared/fsUtils.js'
import { isArray } from '@antd-relx/shared'
import { pathToFileURL } from 'url'

const { pathExistsSync, statSync } = fse

export interface RelxConfig {
    /**
     * @default `Relx`
     * UI library name.
     */
...
export function defineConfig(config: RelxConfig) {
    return config
}
...
export async function getRELXConfig(emit = false): Promise<Required<RelxConfig>> {
    // @ts-ignore
    const defaultConfig = (await import('./relx.default.config.js')).default
    const config: any = pathExistsSync(RELX_CONFIG)
        ? (await import(`${pathToFileURL(RELX_CONFIG).href}?_t=${statSync(RELX_CONFIG).mtimeMs}`)).default
        : {}
